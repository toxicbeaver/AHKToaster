...
Notify(Title="Notify()",Message="",Duration="",Options="")
{
 ...
 static SI,SC,ST,IW,IH,IN,XC,XS,XW,PC,PB,AI,AO
 ...
 If (Options)			; skip parsing steps if Options param isn't used
 {
  ...
  {			; and call it back later with "Style=Load"
   ...
   Saved .= " TS=" TS " TW=" TW " TC=" TC " TF=" TF " MS=" MS " MW=" MW " MC=" MC " MF=" MF "AI=" AI "AO=" AO
...
;—————— end if options ————————————————————————————————————————————————————————————————————————————
  ...
  GC_ := GC_<>"" ? GC_ : GC := GC<>"" ? GC : "111111"		; defaults are set here, and static overrides are used and saved - Color
  GR_ := GR_<>"" ? GR_ : GR := GR<>"" ? GR : 9		; and non static options (with OP_=) are used but not saved - Radius
  GT_ := GT_<>"" ? GT_ : GT := GT<>"" ? GT : "245" ; Transparency
  BC_ := BC_<>"" ? BC_ : BC := BC<>"" ? BC : "000000" ; Border Color
  BK_ := BK_<>"" ? BK_ : BK := BK<>"" ? BK : "Silver" ; Border Flash Color
  BW_ := BW_<>"" ? BW_ : BW := BW<>"" ? BW : 2 ; Border Width
  BR_ := BR_<>"" ? BR_ : BR := BR<>"" ? BR : 13 ; Border Radius
  BT_ := BT_<>"" ? BT_ : BT := BT<>"" ? BT : 105 ; Border Transparency
  BF_ := BF_<>"" ? BF_ : BF := BF<>"" ? BF : 350 ; Border Flash Speed
  TS_ := TS_<>"" ? TS_ : TS := TS<>"" ? TS : 11 ; Title Font Size
  TW_ := TW_<>"" ? TW_ : TW := TW<>"" ? TW : 700 ; Title Font Weight
  TC_ := TC_<>"" ? TC_ : TC := TC<>"" ? TC : "Default" ; Title Font Color
  TF_ := TF_<>"" ? TF_ : TF := TF<>"" ? TF : "Calibri" ; Title Font Face
  MS_ := MS_<>"" ? MS_ : MS := MS<>"" ? MS : 10 ; Message Font Size
  MW_ := MW_<>"" ? MW_ : MW := MW<>"" ? MW : "500" ; Message Font Weight
  MC_ := MC_<>"" ? MC_ : MC := MC<>"" ? MC : "Default" ; Title Font Color
  MF_ := MF_<>"" ? MF_ : MF := MF<>"" ? MF : "Calibri" ; Title Font Face
  SI_ := SI_<>"" ? SI_ : SI := SI<>"" ? SI : 0 ; Speed In
  SC_ := SC_<>"" ? SC_ : SC := SC<>"" ? SC : 0 ; Speed Clicked
  ST_ := ST_<>"" ? ST_ : ST := ST<>"" ? ST : 0 ; Speed Timeout
  IW_ := IW_<>"" ? IW_ : IW := IW<>"" ? IW : 32 ; Image Width
  IH_ := IH_<>"" ? IH_ : IH := IH<>"" ? IH : 32 ; Image Height
  IN_ := IN_<>"" ? IN_ : IN := IN<>"" ? IN : 0 ; Image Icon Number
  XF_ := XF_<>"" ? XF_ : XF := XF<>"" ? XF : "Arial Black" ; X-Button Font Face
  XC_ := XC_<>"" ? XC_ : XC := XC<>"" ? XC : "Default" ; X-Button Color
  XS_ := XS_<>"" ? XS_ : XS := XS<>"" ? XS : 12 ; X-Button Font Size
  XW_ := XW_<>"" ? XW_ : XW := XW<>"" ? XW : 800 ; X-Button Font Weight
  PC_ := PC_<>"" ? PC_ : PC := PC<>"" ? PC : "Default" ; Progress Bar Color
  PB_ := PB_<>"" ? PB_ : PB := PB<>"" ? PB : "Default" ; Progress Bar Background Color
  AI_ := AI_<>"" ? AI_ : AI := AI<>"" ? AI : "0x40008" ; Enter Effect
  AO_ := AO_<>"" ? AO_ : AO := AO<>"" ? AO : "0x50001" ; Exit Effect
...
;————————————————————————————————————————————————————————————————————————
 ...
 {
  If FileExist(Image)
   Gui, %GN%:Add, Picture, w%IW_% h%IH_% Icon%IN_% v_Image_ x8 y8, % Image
  Else
 ...
 If Title <>				; title text control added next, if used
 {
  ...
  Gui, %GN%:Add, Text, %ImageOptions% BackgroundTrans v_Title_ y12, % Title
 }
...
if (GW < 250)
 ...
 If SI_
  DllCall("AnimateWindow","UInt",WinExist(),"Int",SI_,"UInt",AI)		; animated in, if SI is used
 Else
...
_Notify_Kill_25:
 ...
 {
  Gui, %GK%:+LastFound
  DllCall("AnimateWindow","UInt",WinExist(),"Int",ST,"UInt",AO)
 }
 Gui, %GK%:Destroy
